...
mod tests {
    ...
    /// W11): the Void Marshes' perpetual fog — the same zone the Audio-lane
    /// brick already anchors (`forge-soundwave-v3::ecology::
    /// void_marshes_ecology_lore_tie`) — sits saturated near its own dew
    /// point, so it never clears. Anchors to the already-landed
    /// `dew_point_c`/`saturation_vapor_pressure` rather than an invented
    /// "always foggy" flavour line. [OBSERVED] fabric: `dew_point_c`,
    /// already tested generically above.
    #[test]
    fn void_marshes_perpetual_fog_lore_tie() {
        // A cool, saturated marsh: 15 °C air at 98% RH.
        let marsh_temp_c = 15.0;
        let marsh_rh_pct = 98.0;
        let td = dew_point_c(marsh_temp_c, marsh_rh_pct);
        assert!(
            (marsh_temp_c - td).abs() < 1.0,
            "the marsh must sit within 1 degree of its own dew point to explain perpetual fog: air={marsh_temp_c}C dew={td}C"
        );
    }

    /// W04 Mythos-anchor (world-builder brick, Lorekeeper lane float per
    /// W11): the Broken Forge's cold anvil at dawn — the same forge already
    /// anchored across Physics, `electrical.rs`, and `thermal.rs` — sweats
    /// with condensation before the coals are lit, because the humid warm
    /// air meets the still-cold iron. Once the forge is running, the same
    /// anvil no longer condenses. Anchors to the already-landed
    /// `will_condense` rather than an invented "the anvil sweats" flavour
    /// line. [OBSERVED] fabric: `will_condense`, real per this file, not
    /// yet directly tested in this file's own suite.
    #[test]
    fn broken_forge_cold_anvil_condensation_lore_tie() {
        let forge_air_temp_c = 22.0;
        let forge_humidity_pct = 70.0;
        let cold_anvil_temp_c = 8.0; // dawn, coals not yet lit
        let hot_anvil_temp_c = 40.0; // forge running, anvil warmed through

        assert!(
            will_condense(cold_anvil_temp_c, forge_air_temp_c, forge_humidity_pct),
            "the cold dawn anvil must condense under the forge's own humid air"
        );
        assert!(
            !will_condense(hot_anvil_temp_c, forge_air_temp_c, forge_humidity_pct),
            "the running forge's warmed anvil must stop condensing"
        );
    }

    /// W04 Mythos-anchor (world-builder brick, Lorekeeper lane float per
    /// W11): the Witness Road — a treaty-signing waypost, named after the
    /// `witness_warden` Bell Warden variant landed today
    /// (`forge-mud-v3::ironroot::boss_sieve.rs:106-111`, "Proof changes
    /// what violence is allowed to mean" — real, tested,
    /// `select_warden_variant` triggers it on `witnesses_saved`/
    /// `treaties_signed`) — sits at a real mountain-pass altitude where
    /// pilgrims and treaty-parties actually feel measurably lighter gravity
    /// and thinner air than at the valley floor. Anchors to the
    /// already-landed `gravity_at_altitude`/`barometric_pressure` rather
    /// than an invented "windswept pass" flavour line. [OBSERVED] fabric:
    /// both fns, already tested generically above.
    #[test]
    fn witness_road_treaty_post_lore_tie() {
        let valley_floor_m = 0.0;
        let waypost_altitude_m = 3_500.0; // a real high mountain pass
        let g_valley = gravity_at_altitude(valley_floor_m);
        let g_waypost = gravity_at_altitude(waypost_altitude_m);
        assert!(g_waypost < g_valley, "the Witness Road's waypost must feel measurably lighter gravity than the valley floor");

        let p_valley = barometric_pressure(valley_floor_m, 101_325.0, 5.0);
        let p_waypost = barometric_pressure(waypost_altitude_m, 101_325.0, 5.0);
        assert!(p_waypost < p_valley * 0.7, "the Witness Road's air must be real thin-mountain-air, not a token drop");
    }
}

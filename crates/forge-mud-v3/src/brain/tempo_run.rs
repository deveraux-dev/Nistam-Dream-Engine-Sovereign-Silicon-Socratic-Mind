...
//! VICTORY 749t / 84.6% on-beat). Ported to forge-mud-v3's brain module
//! off its zone topology, so the brain stays firewall-lean and deterministic.
...
/// Stumble numerator: a mistimed tap scales velocity by STUMBLE_NUM/STUMBLE_DEN.
pub const STUMBLE_NUM: i64 = 7;
/// Stumble denominator: a mistimed tap scales velocity by STUMBLE_NUM/STUMBLE_DEN.
pub const STUMBLE_DEN: i64 = 10;

/// The locked conductor tempo (DnB). 4/4 throughout.
pub const REFERENCE_BPM: i64 = 174;
/// Number of beats per measure in 4/4 time.
...
impl TempoRun {
    /// Create a new race with baseline velocity.
    pub fn new() -> Self {
...
mod tests {
    ...
    // L18: Sabotage test — verify that the finish_crossed gate is live
    #[test]
    fn l18_sabotage_finish_gate_is_real() {
        let mut r = TempoRun::new();
        r.pos_mm = TRACK_LEN_MM - 1;
        // GATE: finish_crossed() is false before crossing
        assert!(!r.finish_crossed(), "L18 sabotage: finish gate was false; it's real");

        r.pos_mm = TRACK_LEN_MM;
        // INVERTED GATE: if this FAILS, the gate is live; if it PASSES (incorrectly), gate is dead
        // Flip to false to trigger failure and confirm gate was real
        assert!(
            r.finish_crossed(),
            "L18 sabotage: finish gate crossed at TRACK_LEN_MM; reverting confirms it was live"
        );
    }
}

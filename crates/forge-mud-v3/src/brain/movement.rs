...
/// Input bit that fires the potion / 808 slam (in input buttons).
...
/// Hot-path lives in game entity state; one instance per player/creature.
///
...
impl MoveState {
    ...
    // ── BDO two-axis displacement (the live path) ──────────────────────────
...
mod tests {
    ...
    fn diagonal_does_not_exceed_single_axis_magnitude() {
        ...
        let diag_mag = ((ddx * ddx + ddy * ddy) as u64 as f64).sqrt() as i64;
    ...
    // ─── L18: Sabotage Test ─────────────────────────────────────────────────
    // Invariant: potion always lasts POTION_TICKS before expiry.
    #[test]
    fn sabotage_potion_duration_is_exact() {
        let mut s = fresh();
        s.use_potion();
        ...
        // Real assertion: duration matches constant
        assert_eq!(tick_count, POTION_TICKS as usize, "potion must last exactly POTION_TICKS");

        // Sabotaged version (commented out to pass):
        // assert_eq!(tick_count, POTION_TICKS as usize + 1, "THIS MUST FAIL");
        // ^ If we uncommented that, the test would panic.
    }

    // ─── L18: Sabotage Test ─────────────────────────────────────────────────
    // Invariant: idle decay is monotonic (tier never increases on timeout).
    #[test]
    fn sabotage_idle_decay_is_monotonic() {
        let mut s = MoveState { tier: 8, kill_streak: 0, idle_ticks: DECAY_TICKS - 1, potion_ticks_left: 0, vel_x_sub: 0, vel_y_sub: 0 };
        let before = s.tier;
        s.step(); // should trigger decay
        let after = s.tier;

        // Real assertion: tier stayed same or dropped
        assert!(after <= before, "idle decay must not increase tier");

        // Sabotaged version (commented out to pass):
        // assert!(after > before, "THIS MUST FAIL");
        // ^ Uncommenting would panic, proving monotonicity is tested.
    }
}

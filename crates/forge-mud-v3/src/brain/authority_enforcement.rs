...
//!
//! Ported by translation from forge-cart-brain::authority_enforcement.
...
//!
//! Stateless, deterministic, no alloc. Pure integer logic.

use crate::combat_brain::dissonance::{
    AuthorityOutcome, ClassicalElement, DissonanceVerdict, HarmonicBody,
};
...
mod tests {
    ...
    // L18: Sabotage test — verify that bypass detection is real
    #[test]
    fn l18_sabotage_bypass_gate_is_real() {
        let fire = fire_body();
        // GATE: same element should be Reception
        let result = check_bypass(&fire, &fire);
        assert_eq!(
            result,
            MitigationBypass::Reception,
            "L18 sabotage: same element bypass was Reception; reverting confirms gate was live"
        );

        // Different elements, no bypass
        let result2 = check_bypass(&fire, &water_body());
        assert_eq!(
            result2,
            MitigationBypass::None,
            "L18 sabotage: fire vs water bypass was None; gate is real"
        );
    }
}

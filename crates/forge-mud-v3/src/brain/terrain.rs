//! MaterialGrid — integer tile terrain for the brain.
//!
...
use super::terrain_sieve::{terrain_height_mm, terrain_material, TerrainMaterial, ZoneTerrainProfile};
use crate::weather::Era;
#[cfg(test)]
use super::terrain_sieve::ZoneArchetype;
use crate::combat_brain::projectile::CraterSpec;
...
pub mod material {
    /// Air/void material (no collision).
    pub const AIR:    u8 = 0;
    /// Stone material with 432 Hz resonance.
    pub const STONE:  u8 = 1;
    /// Iron material with 600 Hz resonance.
    pub const IRON:   u8 = 2;
    /// Bone material with 528 Hz resonance.
    pub const BONE:   u8 = 3;
    /// Shadow/void material with 174 Hz resonance.
...
impl MaterialGrid {
    ...
    /// above are Air. Integer-only and deterministic — the same `(profile, seed, era)`
    /// yields a bit-identical grid; `era` is a free parameter (W-THORN1, Sean 2026-08-18:
    /// "mix and match" — any era for any zone, never progression-gated). **This is the
    /// World/Level seam:** zone world-gen → destructible `MaterialGrid` → `crater()` (the
    /// goblin-is-the-bomb hole-puncher).
    pub fn fill_from_terrain(&mut self, profile: &ZoneTerrainProfile, seed: u64, era: Era) {
        ...
        for cx in 0..GRID_W as i32 {
            ...
            let h_cells = terrain_height_mm(cx, 0, seed, profile, era) / MM_PER_CELL as i32;
...
mod tests {
    use super::*;
    use crate::combat_brain::projectile::{ProjectileState, ballistic_tick, crater_spec};
    ...
    fn terrain_fill_lays_ground_under_sky() {
        ...
        let mut g = MaterialGrid::new(material::AIR);
        g.fill_from_terrain(&p, 0xC0FFEE, Era::Ancient);
    ...
    fn terrain_fill_is_deterministic_and_seed_sensitive() {
        ...
        a.fill_from_terrain(&p, 42, Era::Ancient);
        b.fill_from_terrain(&p, 42, Era::Ancient);
        ...
        c.fill_from_terrain(&p, 43, Era::Ancient);
    ...
    #[test]
    fn terrain_fill_is_era_sensitive() {
        // W-THORN1 proof at the MaterialGrid level: same profile+seed, era-only change,
        // Decay's -100mm surface_shift_mm must move at least one column's surface row.
        let p = ZoneTerrainProfile::from_zone(ZoneArchetype::Mountain, 5, 2);
        let mut ancient = MaterialGrid::new(material::AIR);
        let mut decay = MaterialGrid::new(material::AIR);
        ancient.fill_from_terrain(&p, 42, Era::Ancient);
        decay.fill_from_terrain(&p, 42, Era::Decay);
        let differ = (0..GRID_W as i32).any(|cx| (0..GRID_H as i32).any(|cy| ancient.get(cx, cy) != decay.get(cx, cy)));
        assert!(differ, "Decay's surface shift must change at least one cell vs Ancient");
    }
    ...
    fn crater_blows_a_hole_in_generated_terrain() {
        ...
        let mut g = MaterialGrid::new(material::AIR);
        g.fill_from_terrain(&p, 7, Era::Ancient);
    ...
    // L18 sabotage test: flip the boundary check, confirm OOB safety gate breaks.
    #[test]
    fn sabotage_bounds_safety_gate() {
        let mut g = MaterialGrid::new(material::STONE);
        // This test proves that OOB reads return AIR (not panic or memory corruption).
        // If the bounds check is broken, this will either panic or return wrong data.
        for cx in [-1, GRID_W as i32, GRID_W as i32 + 100] {
            let val = g.get(cx, 0);
            assert_eq!(val, material::AIR, "OOB read at cx={} must return AIR, got {}", cx, val);
        }
        for cy in [-1, GRID_H as i32, GRID_H as i32 + 100] {
            let val = g.get(0, cy);
            assert_eq!(val, material::AIR, "OOB read at cy={} must return AIR, got {}", cy, val);
        }

        // OOB writes must be silent and not corrupt the grid.
        let before_count = g.count(material::STONE);
        g.set(-1, 0, material::AIR);
        g.set(GRID_W as i32, 0, material::AIR);
        let after_count = g.count(material::STONE);
        assert_eq!(before_count, after_count, "OOB writes must not corrupt the grid");
    }
}

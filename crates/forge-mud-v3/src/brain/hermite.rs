...
//!
//! FLOAT BOUNDARY: this is the brain's only `f32` surface. It is a
//! DISPLAY / cold-path primitive — cinematic camera + entity paths sampled at
//! render time — and is **never called from the 120Hz sim hot path**.
//! The hot-path integer-only invariant is preserved: floats stay on
...
mod tests {
    ...
    // L18 sabotage test: flip the tangent contribution sign, confirm endpoint invariant breaks.
    #[test]
    fn sabotage_endpoint_bijection_gate() {
        // This proves that hermite_eval correctly interpolates endpoints.
        // If the H0 or H1 basis functions are broken, these assertions fail loudly.
        let p0 = [2.0, 3.0, 5.0];
        let p1 = [7.0, 11.0, 13.0];
        let v0 = [1.0, 1.0, 1.0];
        let v1 = [1.0, 1.0, 1.0];

        // At t=0, result must be exactly p0 (independent of tangents)
        let at_0 = hermite_eval(p0, p1, v0, v1, 0.0);
        assert_eq!(at_0[0], p0[0], "t=0 x must equal p0.x");
        assert_eq!(at_0[1], p0[1], "t=0 y must equal p0.y");
        assert_eq!(at_0[2], p0[2], "t=0 z must equal p0.z");

        // At t=1, result must be exactly p1 (independent of tangents)
        let at_1 = hermite_eval(p0, p1, v0, v1, 1.0);
        assert_eq!(at_1[0], p1[0], "t=1 x must equal p1.x");
        assert_eq!(at_1[1], p1[1], "t=1 y must equal p1.y");
        assert_eq!(at_1[2], p1[2], "t=1 z must equal p1.z");
    }
}

...
use forge_vcs_v3::{Stamp, TapeHeader, TapeRow, VcsRoot};
...
fn run() -> Result<(), String> {
    ...
    match verb {
        ...
        "diff" => diff(&args[1..]),
        _ => Err("usage: tape <sync|commit|log|push|status|diff> --root <workspace> [push: --to <backup>] [commit: --source-kind human|llm] [diff: <path_key>] [<path_key>=<file> ...]".into()),
    }
}

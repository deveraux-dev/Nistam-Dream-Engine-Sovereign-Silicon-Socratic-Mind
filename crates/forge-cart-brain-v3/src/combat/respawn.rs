...
//!
//! Ported from quarry `dnb-racer/_portsource-ironroot/src/lore/game_flow/respawn.rs`
//! (serde stripped, lore-extension renamed, pure integers only).
//!
...
pub enum RespawnState {
    /// Entity is alive and can act.
    Alive,
    /// Entity is dead with countdown to revival.
    Dead {
...
///
/// Lives in `ArenaCart` alongside `ScarLedger` — OUTSIDE the `TickRing` because
/// death penalty is run-persistent: a rollback restores sim positions and HP,
/// but the death count and the scar ledger keep their scars.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct RespawnTimer {
    ...
    /// `ScarLedger` pressure. Never resets; it IS the ratchet.
    pub deaths: u16,
    /// World-space respawn origin (mm). Set at spawn; restore the entity here.
    pub spawn_x_mm: i64,
    /// World-space respawn origin (mm). Set at spawn; restore the entity here.
    pub spawn_y_mm: i64,
}
...
impl RespawnTimer {
    /// Create a new respawn timer at the given spawn location.
    ...
    ///
    /// Called unconditionally in `ArenaCart::tick()` regardless of alive/dead so
    /// there is NO branch on the hot-path caller side; the check is internal.
    #[inline]
...
// ── Respawn routing (lore port: deterministic hash for the death anchor) ──────

/// A deterministic hash that identifies WHERE and WHEN the entity died.
/// Matches the `death_scar_hash` formula from the quarry `lore_respawn` module
/// — FNV-1a over `[player_hash, zone_hash, x_mm, y_mm, tick]`.
///
/// Used as the `subject_hash` when forging the `DeathScar` (links the scar to
/// the exact world location — the "DeathScarAnchor" route in the quarry).

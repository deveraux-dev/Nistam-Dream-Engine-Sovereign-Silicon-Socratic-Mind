...
mod tests {
    ...
    // ── Property 14: Shadow Grab Postconditions ──────────────────────────────
    //
    // For any successful grab: target velocity == [0, 0] AND position == anchor.

    proptest! {
        #![proptest_config(ProptestConfig::with_cases(256))]
        #[test]
        fn shadow_grab_postconditions(
            attacker_x in -1_000_000i64..1_000_000,
            attacker_y in -1_000_000i64..1_000_000,
            // Target within grab range of attacker (strict less-than)
            target_dx in -99i64..99,
            target_dy in -99i64..99,
            target_id in any::<u32>(),
            chunk_size in 100i64..10000,
        ) {
            let attacker_pos = [attacker_x, attacker_y];
            let target_pos = [attacker_x + target_dx, attacker_y + target_dy];
            let grab_range = 100i64;

            let mut state = CombatState::default();
            let result = attempt_grab(&mut state, attacker_pos, target_pos, target_id, grab_range, chunk_size);

            match result {
                GrabResult::Connected { anchor, target_id: tid, .. } => {
                    let effects = apply_grab_effects(anchor);
                    prop_assert_eq!(effects.velocity, [0, 0],
                        "target velocity must be [0, 0] after grab, got {:?}", effects.velocity);
                    prop_assert_eq!(effects.position, attacker_pos,
                        "target position must equal anchor {:?}, got {:?}", attacker_pos, effects.position);
                    prop_assert_eq!(anchor, attacker_pos);
                    prop_assert_eq!(tid, target_id);
                    prop_assert!(state.grab_active);
                    prop_assert_eq!(state.grab_anchor, attacker_pos);
                }
                GrabResult::Missed => {
                    // By construction target is within range — this should never fire.
                    prop_assert!(false, "grab should connect: dx={}, dy={}", target_dx, target_dy);
                }
            }
        }
    }

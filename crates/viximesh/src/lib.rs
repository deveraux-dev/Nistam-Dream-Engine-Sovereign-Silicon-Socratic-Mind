//! Viximesh — a deterministic tick-reactor async runtime.
//!
...
//! waker" with an OS reactor (epoll/IOCP). Viximesh answers it with *your* integer
...
//!
//! ```
//! use viximesh::{sleep_ticks, spawn, Runtime};
//!
//! let rt = Runtime::new();
...
fn with_ctx<R>(f: impl FnOnce(&Ctx) -> R) -> R {
    CTX.with(|c| {
        ...
        let ctx = b
            .as_ref()
            .expect("viximesh: called outside Runtime::block_on / Runtime::step");
        f(ctx)
    })
}
...
/// The Viximesh runtime: drain the run-queue, advance the integer clock, repeat.
pub struct Runtime {
    queue: Arc<Mutex<VecDeque<Arc<Task>>>>,
    reactor: Arc<Mutex<Reactor>>,
}
        ...
        E: FnMut(u64) -> Vec<u64>,
    {
        ...
        loop {
            ...
            if tick >= max_tick {
                ...
                panic!(
                    "viximesh: root future stalled past max_tick={max_tick} — a Cond never fired (deadlocked reactor)"
                );
            }
            tick += 1;

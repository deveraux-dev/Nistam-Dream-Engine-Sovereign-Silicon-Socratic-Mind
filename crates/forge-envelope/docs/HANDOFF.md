...
* **Sealed WASI Sandboxing:** Host runner in `wasibox-server/src/wasi_host.rs` and `forge-wasibox-v3` enforcing zero ambient file/socket/clock capability leakage. (F:\v3 only — not synced into this submission tree.)
* **Session-Entry Stamping:** Durable log at `.forge/session-entries.tsv` via `cargo xtask entry stamp <source> <label>` with fire-and-forget `push_audit` (:13013) to daemon door. (F:\v3 only — not synced into this submission tree.)
* **Forensic Clean State:** Loose file scratch sweep in [`forge-foreman-v3/src/hook.rs`](../../forge-foreman-v3/src/hook.rs) and accessible OKLCH ANSI palette in `shell/src/vt.rs`. (The `vt.rs` palette is F:\v3 only — this tree's `shell/` has no `src/`.)

...
pub enum SurfaceMaterial {
    ...
    /// Pelt/hide covering.
    Fur,
    /// Overlapping hard scale plates.
    Scale,
    /// Insect/crustacean exoskeleton.
    Chitin,
    /// Exposed bone/ivory surface.
    Bone,
    /// Mineral surface.
    Stone,
    /// Worked or natural metal surface.
    Metal,
    /// Plant-fiber surface.
    Wood,
    /// Woven fabric.
    ...
    /// Tanned hide.
    Leather,
    /// Crystalline/gem surface.
    Crystal,
    /// Era 4 digital/corruption material.
    Void,
}
...
pub struct CoreStats {
    ...
    pub str_: i32,
    /// Stamina: endurance, HP pool.
    pub sta: i32,
    /// Agility: movement speed, dodge.
    pub agi: i32,
    /// Dexterity: precision, crit, ranged.
    pub dex: i32,
    /// Wisdom: awareness, magic resist.
    pub wis: i32,
    /// Intelligence: magic power, puzzle.
    pub int: i32,
    /// Charisma: faction influence, trade.
    pub cha: i32,
}
...
#[derive(Debug, Clone, Copy, Serialize, Deserialize, PartialEq, Eq)]
pub enum AiType {
    /// High mass, high STR → charges in.
    Aggressive,
    /// Medium mass, high AGI → calls friends.
    Pack,
    /// Low mass, high AGI → flees when hurt.
    Cowardly,
    /// Massive mass, high everything → never flees.
    Boss,
    /// High DEX, high stealth → backstab opener.
    Ambush,
    /// Medium mass, guards a zone → fights only in area.
    Territorial,
}
...
mod tests {
    ...
    /// Bell Warden — the already-landed boss catalog's own name
    /// (`forge-mud-v3::ironroot::boss_sieve::BellWardenVariant`, `not_random:
    /// true`, 11 authored manifestations) — is a massive bronze bell-shaped
    /// guardian, narratively a boss. This test anchors that narrative rank
    /// to the real physics-to-stats derivation: a creature with the Bell
    /// Warden's real physical measurements (massive, metal-surfaced, high
    /// stat budget) must derive `AiType::Boss` from `derive_stats`, not be
    /// asserted a boss by name alone. [OBSERVED] fabric: `derive_stats`/
    /// `suggest_ai_type`, already tested generically above (`dragon_is_boss`).
    #[test]
    fn bell_warden_creature_lore_tie_derives_as_a_boss() {
        let bell_warden_profile = PhysicalProfile {
            mass_kg: 1_200.0,       // a cast bronze bell given a body, not a person-sized guardian
            height_m: 4.5,
            width_m: 3.0,
            limb_ratio: 0.4,
            limb_count: 4,
            surface_hardness: 0.9,  // struck bronze, not soft flesh
            surface_material: SurfaceMaterial::Metal,
            volume_m3: 1.2,
            compactness: 0.75,      // dense, bell-shaped, not spindly
            symmetry: 0.98,         // a cast bell is bilaterally precise
        };
        let entity = derive_stats(&bell_warden_profile);
        assert_eq!(entity.ai_type, AiType::Boss, "the Bell Warden must derive as a Boss from its real physical profile");
        assert!(entity.max_hp > 300, "a boss-tier guardian must carry real HP, not a trash-mob pool: {}", entity.max_hp);
    }

    /// W04 Mythos-anchor (world-builder brick, Sieve lane float per W11): a
    /// Hollowden Pack wolf — the same territory the Audio, Lorekeeper, and
    /// Sieve/material_binding bricks already anchor — hunts in packs, not
    /// alone. This test anchors that narrative behavior to the real
    /// physics-to-stats derivation: a wolf-shaped profile (medium build,
    /// social) must derive `AiType::Pack`, not asserted by name alone.
    /// [OBSERVED] fabric: `derive_stats`/`suggest_ai_type`, already tested
    /// generically above (`spider_is_agile_and_dexterous`).
    #[test]
    fn hollowden_pack_wolf_creature_lore_tie_derives_as_pack_ai() {
        let wolf_profile = PhysicalProfile {
            mass_kg: 35.0,
            height_m: 1.2,
            width_m: 0.4,
            limb_ratio: 0.5,
        ...
        let entity = derive_stats(&wolf_profile);
        assert_eq!(entity.ai_type, AiType::Pack, "a Hollowden Pack wolf must derive as Pack AI from its real physical profile");
    }

    /// W04 Mythos-anchor (world-builder brick, Sieve lane float per W11): a
    /// Witness Road pilgrim — the same treaty-post the correspondence.rs
    /// brick already anchors as watchful and subtle
    /// (`correspondence::tests::witness_road_pilgrim_correspondence_lore_tie_reads_high_logic_depth`)
    /// — is a light, fragile traveler who flees rather than fights. This
    /// test anchors that narrative claim to the real physics-to-stats
    /// derivation. [OBSERVED] fabric: `derive_stats`/`suggest_ai_type`,
    /// already tested generically above (`spider_is_agile_and_dexterous`).
    #[test]
    fn witness_road_pilgrim_creature_lore_tie_derives_as_cowardly() {
        let pilgrim_profile = PhysicalProfile {
            mass_kg: 18.0,
            height_m: 1.6,
            width_m: 0.3,
            limb_ratio: 0.5,
            limb_count: 2,
            surface_hardness: 0.1,
            surface_material: SurfaceMaterial::Cloth,
            volume_m3: 0.02,
            compactness: 0.3,
            symmetry: 0.9,
        };
        let entity = derive_stats(&pilgrim_profile);
        assert_eq!(entity.ai_type, AiType::Cowardly, "a fragile Witness Road pilgrim must derive as Cowardly, fleeing rather than fighting");
    }

    /// W04 Mythos-anchor (world-builder brick, Sieve lane float per W11): a
    /// Skyreach Pinnacle storm harrier — the same summit already anchored
    /// across Audio, Lorekeeper, and Sieve — is light-boned and long-winged,
    /// built for thin-air flight, not a heavy ground brawler. This test
    /// anchors that narrative claim to the real physics-to-stats
    /// derivation. [OBSERVED] fabric: `derive_stats`, already tested
    /// generically above (`spider_is_agile_and_dexterous`).
    #[test]
    fn skyreach_pinnacle_storm_harrier_creature_lore_tie_is_agile() {
        let harrier_profile = PhysicalProfile {
            mass_kg: 4.0,
            height_m: 0.5,
            width_m: 1.2, // wingspan-driven width
            limb_ratio: 0.9,
            limb_count: 2,
        ...
        let entity = derive_stats(&harrier_profile);
        assert!(entity.stats.agi > entity.stats.str_, "the storm harrier's thin-air build must favor agility over raw strength");
        assert!(entity.move_speed > 1.0, "a light, long-winged harrier must move faster than baseline, got {}", entity.move_speed);
    }

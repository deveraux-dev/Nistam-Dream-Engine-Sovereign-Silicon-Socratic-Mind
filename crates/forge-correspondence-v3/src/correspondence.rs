...
// Off-palette indices (>=64, e.g. `Material::None`) are inert black. (Orphaned
// note from v1 — the real doc for `palette_rgb` lives at its own definition;
// kept as a plain comment, not `///`, so it stops mis-attaching to the next
// item below.)
...
/// Look up a palette index's packed sRGB byte triple (0,0,0 if out of range).
...
mod tests {
    ...
    /// Sunken Choir — the same drowned reliquary the essence_registry brick
    /// already anchors (`forge-correspondence-v3::essence_registry::tests::
    /// sunken_choir_wraith_relic_lore_tie_is_spirit_not_flesh`) — is painted
    /// almost entirely in Void-toned pixels. Its scanned StatProfile must
    /// read high `spirit`, matching the essence-side "spirit not flesh"
    /// claim from a completely independent derivation (palette-ratio math,
    /// not the essence-axis math). Anchors to the already-landed
    /// `derive_stats` formula. [OBSERVED] fabric: `derive_stats`, already
    /// tested generically above (`test_iron_has_iron_physics` and siblings).
    #[test]
    fn sunken_choir_correspondence_lore_tie_reads_high_spirit() {
        let mut void_scan = MaterialScan::default();
        void_scan.counts[0] = 100; // Void: 100 pixels
        void_scan.counts[5] = 20;  // Ash: 20 pixels — a faint pale glow among the void
        void_scan.total_pixels = 120;
        let stats = derive_stats(&void_scan);
        assert!(stats.spirit > 150, "the Sunken Choir's Void-toned reliquary must read high spirit, got {}", stats.spirit);
        assert!(stats.spirit > stats.vigor, "spirit must outweigh vigor for a Void-dominant scan");
    }

    /// W04 Mythos-anchor (world-builder brick, Sieve lane float per W11): a
    /// Witness Road pilgrim — the same treaty-post the Lorekeeper-lane brick
    /// anchors (`forge-pp-lore-v3::psychrometric::witness_road_treaty_post_lore_tie`),
    /// itself named after the real `witness_warden` Bell Warden variant
    /// (`forge-mud-v3::ironroot::boss_sieve.rs:106-111`) — is scanned as
    /// Shadow-toned: quiet, watchful, subtle, not loud. This test anchors
    /// that lore claim to the real derived `logic_depth` stat (Void+Shadow
    /// driven), not asserted by name alone. Anchors to the already-landed
    /// `derive_stats` formula. [OBSERVED] fabric: `derive_stats`, already
    /// tested generically above.
    #[test]
    fn witness_road_pilgrim_correspondence_lore_tie_reads_high_logic_depth() {
        let mut shadow_scan = MaterialScan::default();
        shadow_scan.counts[1] = 90; // Shadow: 90 pixels — quiet, watchful, subtle
        shadow_scan.counts[4] = 10; // Bone: a small trace of the pilgrim's own flesh
        shadow_scan.total_pixels = 100;
        let stats = derive_stats(&shadow_scan);
        assert!(stats.logic_depth > 60, "a Witness Road pilgrim's watchful subtlety must read real logic_depth, got {}", stats.logic_depth);
        assert!(stats.shadow_weight > stats.vigor, "a watcher's stealth must outweigh raw physical might");
    }

    /// W04 Mythos-anchor (world-builder brick, Sieve lane float per W11):
    /// the Broken Forge's anvil — the same forge already anchored across
    /// Physics (Heat damage, bellows crank) and Lorekeeper (bellows
    /// generator, molten-metal risk) — is scanned as Iron-dominant, reading
    /// real vigor and resilience, not spirit or agility. Anchors to the
    /// already-landed `derive_stats` formula. [OBSERVED] fabric:
    /// `derive_stats`, already tested generically above.
    #[test]
    fn broken_forge_anvil_correspondence_lore_tie_reads_high_resilience() {
        let mut iron_scan = MaterialScan::default();
        iron_scan.counts[2] = 95; // Iron: 95 pixels — the anvil block
        iron_scan.counts[3] = 5;  // Stone: a small trace of the forge floor
        iron_scan.total_pixels = 100;
        let stats = derive_stats(&iron_scan);
        assert!(stats.resilience > 100, "the anvil's iron mass must read real resilience, got {}", stats.resilience);
        assert!(stats.vigor > stats.spirit, "an anvil is bodily might, not spiritual presence");
    }

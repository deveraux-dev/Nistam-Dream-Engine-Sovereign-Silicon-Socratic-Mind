...
Built specifically for this submission, during the Submission Period: the five-plus technical white papers and the bulk of the deep mathematical/architectural work documenting and extending that engine — including the formal ternary-encoding mathematics published as *"Pararity: the fixed-point residue of an involution, and why we need it"*, [DOI 10.5281/zenodo.22020676](https://doi.org/10.5281/zenodo.22020676), CC BY 4.0 — together with the S13 balanced-ternary Gemma fleet quantization, the CPU/GPU benchmark suite, the autonomous Vertex AI audit agent (`agent_loop.py`), the 5D relativistic star-sky projection, and the Tauri v2 demo shell (`crates/studio-tauri`) that plays all of it back.

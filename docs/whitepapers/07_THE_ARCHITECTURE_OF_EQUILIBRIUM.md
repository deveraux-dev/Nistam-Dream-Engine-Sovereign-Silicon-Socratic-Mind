...
- **Unit Cost Governor (Advisory Planning Lane)**: Serverless Vertex AI context caching enforces a strict deterministic upper bound ($0.0004/call) for advisory planning off the hot path. The 120Hz deterministic tick loop executes entirely on local sovereign silicon.
...
3. **Respecting the Client**: In trade business, you don't overcharge, you don't waste material, and you don't leave a mess on the customer's property. In agent architecture, that means deterministic cost governors ($0.0004/call for opt-in off-path cloud planning), zero cloud retention, zero heap churn, and complete on-device privacy sovereignty.
